...
namespace Comet.Game.Database.Repositories
{
    public static class MapsRepository
    {
        ...
        {
            await using var db = new ServerDbContext();
            return await db.Maps.Where(x => x.ServerIndex == -1 || x.ServerIndex == Kernel.Configuration.ServerIdentity)
                .ToListAsync();
        }
        ...
        {
            await using var db = new ServerDbContext();
            return await db.DynaMaps.Where(x => x.ServerIndex == -1 || x.ServerIndex == Kernel.Configuration.ServerIdentity)
                .ToListAsync();
        }
    }
}